...
<template>
  <div class="w-full">
    <form
      class="flex flex-col justify-start w-full bg-gray-950 p-3 text-white rounded-[10px]"
      @submit="addCard"
      v-if="!displayForm"
    >
      <textarea
        class="bg-gray-600 text-2xl p-3 rounded-[3px] outline-0 focus:border-2 border-blue-600"
        v-model="cardName"
        id="card-name"
        type="text"
      ...
      <div class="flex justify-start align-middle pt-4">
        <button class="bg-blue-600 p-2 text-[1.4rem] rounded-[3px] cursor-pointer" type="submit">
          Adicionar cartão
        </button>
        <button
